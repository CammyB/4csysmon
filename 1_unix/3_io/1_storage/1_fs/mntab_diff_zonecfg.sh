...
#
#	5. Modified to run as single script and loop through running zones #2015/08/31
...
for ZONE in `zoneadm list -cv | grep running | awk {'print $2'} | grep -v global`; 
do
...
#        cat $FINAL | mailx -s "$HOST P1: Check mounted" $SERVICEDESKMAIL
#       (sh $SMS $ONSITESMS "$HOST P1: Check mounted `wc -l $FINAL` differences")
#       (sh $SMS $PATRICKSMS "$HOST P1: Check mounted `wc -l $FINAL` differences")
